...
def grid_to_gtsp_instance(matrix, filename, is_simple):
	"""
	This function generates a GTSP problem file which
	will be fed to the LKH solver
	:param matrix: A numpy 2d array of edge weights
	:param filename: Filenames for GTSP problems
	:param is_simple: use simple format?
	:return: None. GLKH will generate a file with results
	"""
	...
	num_clusters = matrix.shape[0]/NUM_NODES_IN_CLUSTER
	...
	if is_simple:
		...
		with open(filename+".txt", "w") as f:
			...
			for i in range(num_clusters):
				row = ""
				for j in range(NUM_NODES_IN_CLUSTER):
					row += "%d "%((NUM_NODES_IN_CLUSTER*i+j)+1)
		...
		os.system("mv "+filename+".gtsp "+GLKH_LOCATION)
		os.system("mv "+filename+".par "+GLKH_LOCATION+"TMP/")


		cur_dir = os.getcwd()
		os.chdir(GLKH_LOCATION)
		cmd = "./GLKH "+"TMP/"+filename+".par"
